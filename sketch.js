...
// Recalculate the dots and image layout when the window is resized
function windowResized() {
  resizeCanvas(800, 800);  // Reset canvas size
  calculateImageAndDots();  // Recalculate the dots
}
...
function calculateImageAndDots() {
  ...
  gridWidth = floor(img.width / xStep) * xStep; // Ensure it fits with the grid
  gridHeight = floor(img.height / yStep) * yStep;
  ...
  for (let i = 0; i < img.width; i += xStep) {
    for (let j = 0; j < img.height; j += yStep) {
      ...
      dots.push(new Dot(i, j, pixelColor, size)); // Store dot
    }
  }
}
...
class Dot {
  constructor(x, y, color, size) {
    ...
    this.color = color;
    // Normalize size based on brightness, keeping it within a fixed range
...
function moveSnake() {
  ...
  if (rowDirection === 'RIGHT') head.x += xStep;
  if (rowDirection === 'LEFT') head.x -= xStep;
